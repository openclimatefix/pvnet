...
class PVNetLightningModule(pl.LightningModule):
    ...
    def validation_step(self, batch: TensorBatch, batch_idx: int) -> None:
        ...
        # Calculate the persistence losses - we only need to do this once per training run
        # not every epoch
        if self.current_epoch == 0:
            ...
            batch_indices = torch.arange(history_data.shape[0], device=history_data.device)
            last_valid_values = history_data[batch_indices, last_valid_indices]

            y_persist = last_valid_values.unsqueeze(1).expand(-1, self.model.forecast_len)
